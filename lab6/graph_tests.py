...
def test_fix_Q():
    ...
    Q = range(MIN_DISTANCE, MAX_DISTANCE * m)  # coefficient of the alleged best way
    ...
    for j in (0, 1, 2):# это  нужно чисто для визуализации
        for t in range(TIMES):#если убрать это будут обрывы, что не очень хорошо
            for q in Q:
                st = time.process_time()
                res = best_way(m, e, D, gen, a, b, p, q)
...
def test_fix_e():
    ...
    m = 5  # amount of ants and cities
    Q = MAX_DISTANCE * m // 2  # coefficient of the alleged best way
